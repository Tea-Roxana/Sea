...
def remove_hits(ship_hits, board, buttons, player_turn):
    ...
    if ship_destroyed:
        ...
        for r, c in cells_to_mark:  #отмечаем клетки как промахи
            board[r][c] = -1
            if player_turn == "player":
                buttons[r][c].config(bg='white', text='•')  #поле ИИ
                player_moves.append((r, c))
            else:
                buttons[r][c].config(bg='light gray', text='•')  #поле игрока
...
def check_winner(turn):
    ...
    new_game = False
    if turn == "player" and not player_ships:
        messagebox.showinfo("Конец игры", "Победил Игрок!") 
        new_game = True
    elif turn == "ai" and not ships:
        messagebox.showinfo("Конец игры", "Победил Компьютер!")
        new_game = True
    if new_game:
        result = messagebox.askyesno("Новая игра", "Начать заново?")
        if result:
            restart()
        return True
    return False
